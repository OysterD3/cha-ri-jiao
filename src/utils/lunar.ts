 ...
 * Credits: https://github.com/laocaixw/calendar/blob/master/%E6%8F%92%E4%BB%B6%E5%BC%80%E5%8F%91%E4%BB%A3%E7%A0%81/plugin/components/calendar/lunar.js
 */
const lunarInfo = [
  0x04bd8,
  0x04ae0,
 ...
 * 检查年份是否输入正确
 */
const _checkYear = (year: number): boolean => {
  if (year < minYear) {
 ...
 * 检查月份是否输入正确
 */
const _checkMonth = (month: number): boolean => {
  if (month < 1) {
 ...
 * 检查日期是否输入正确
 */
const _checkDay = (day: number): boolean => {
  if (day < 1) {
 ...
 */
export const getLunarLeapMonth = (year: number): number => {
  if (_checkYear(year)) {
 ...
 */
export const getLeapMonthDaysCount = (year: number): number => {
 ...
 * 返回农历year年的总天数
 */
export const getLunarYearDaysCount = (year: number): number => {
  if (_checkYear(year)) {
 ...
 * 返回农历year年month月的天数
 */
export const getLunarYearMonthDaysCount = (
  year: number,
  month: number,
): number => {
 ...
 * 农历日期的中文字符串
 */
export const getLunarDayString = (day: number): string => {
  if (_checkDay(day)) {
    const nStr1 = [
      "日",
      "一",
 ...
 */
export const getLunarTermDay = (year: number, n: number): number => {
  if (_checkYear(year) && n <= 23 && n >= 0) {
    ...
    const offDate = new Date(
      31556925974.7 * (year - minYear) +
        sTermInfo[n] * 60000 +
        Date.UTC(minYear, 0, 6, 2, 5),
    );
...
export const solarToLunar = (
  ...
  if (_checkYear(year) && _checkMonth(month) && _checkDay(day)) {
    ...
    let offset = parseInt(
      String((objDate.getTime() - baseDate.getTime()) / 86400000),
    ); // 偏移天数 60 * 60 * 24 * 1000 = 86400000，1天的毫秒数
    let temp = 0;
    let i: number;
 ...
 * 阳历某个月份天数
 */
export const getSolarMonthDaysCount = (year: number, month: number): number => {
  if (_checkYear(year) && _checkMonth(month)) {
    if (month == 2) {
 ...
 * 获取指定日期是阳历年中的第几天
 */
export const getSolarDayNumber = (
  year: number,
  month: number,
  day: number,
): number => {
